<template>
    <div>
        ...
        <div class="media mt-3" v-for="(reply,index) in replies">
            <a class="pr-3" href="#">
                <img class="mr-2 rounded-circle" :src="reply.avatar" alt="Generic placeholder image" style="height:50px;width:50px">
            </a>
            <div class="media-body border-0">
                ...
                <p class="text-muted small">{{reply.reply}}</p>
            </div>
        </div>
...
export default {
    ...
    methods:{
        load_replies(){
            axios.get('/api/service/'+this.id+'/reply')
            .then((response) => {
        ...
        load_more_replies(nextPage){
            axios.get('/api/service/'+this.id+'/reply?page='+this.nextPage)
            .then((response) => {
        ...
        reply_comment(){
            this.$validator.validateAll('valid_reply_form').then((result) => {
                if(result){
                    axios.post('/api/service_review_replies',this.create_reply,{
      ...
      //  Edit
         edit(id,index){
             this.reply_id = id;
             axios.get('/api/service_review_replies/'+id)
             .then(response=>{
       ...
       //  update
        reply_update(id){
            this.$validator.validateAll('reply_update').then((result) => {                  
                if(result){
                    axios.patch('/api/service_review_replies/'+id,this.update_reply,{
                    headers : { Authorization : localStorage.getItem("token")}
                    })
                    .then(response=>{
         ...
         */  
         destory(id,index){
            ...
            if(confirmBox == true){
                axios.delete('/api/service_review_replies/'+id,{
                    headers : { Authorization : localStorage.getItem("token")}
                }).then(response=>{
                    //  Flash Message  
                        this.load_replies();
