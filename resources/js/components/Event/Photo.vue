    ...
    <div>
        <div class="row p-3">
            ...
            <div class="col-md-3 col-sm-4 col-6 py-2" v-for="(event_photo,index) in photos" v-if="index < 7">
                <div class="photo lazyload" @click="photo_modal(index)"  :data-bgset="'/storage/Event/Photos/'+event_photo.thumb"  data-sizes="auto"></div>
            </div>
            <!-- More Photos -->
            ...
            <div class="modal-dialog modal-lg modal_top" role="document">
                <div class="modal-content">
                    <div class="modal-body carousel" v-if="modal_status">
                        <div class="slide" style="height:55vh" v-bind:style='{ backgroundImage: `url(/storage/Event/Photos/${single_photo.path})`}'></div>
                    </div>
                </div>
            </div>
        </div>
