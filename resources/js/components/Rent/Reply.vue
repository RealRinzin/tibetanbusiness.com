...
export default {
    ...
    methods:{
        ...
        load_more_replies(nextPage){
            axios.get('/api/rent/'+this.id+'/reply?page='+this.nextPage)
            .then((response) => {
        ...
        reply_comment(){
            this.$validator.validateAll('valid_reply_form').then((result) => {
                if(result){
                    axios.post('/api/rent_comment_replies',this.create_reply,{
       ...
       //  update
        reply_update(id){
            this.$validator.validateAll('reply_update').then((result) => {                  
                if(result){
                    axios.patch('/api/rent_comment_replies/'+id,this.update_reply,{
                    headers : { Authorization : localStorage.getItem("token")}
                    })
                    .then(response=>{
         ...
         */  
         destory(id,index){
            ...
            if(confirmBox == true){
                axios.delete('/api/rent_comment_replies/'+id,{
                    headers : { Authorization : localStorage.getItem("token")}
                }).then(response=>{
                    //  Flash Message  
                        this.load_replies();
