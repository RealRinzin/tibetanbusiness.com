                ...
                <div class="position-fixed rounded w-75 d-block d-sm-none" style="bottom:10px;z-index:13">
                    <div class="alert alert-secondary alert-dismissible fade show m-1 rounded text-center w-100 p-0" role="alert" id="mobile_add_promote_link">
                        <div class="d-flex flex-row">
                            <div class="w-100 lazyload" :data-bgset="'/storage/Event/Banner/'+popup.banner"  style="background-size:cover;height:auto" data-sizes="auto">
                            </div>
                            <div class="w-100 text-left px-1 py-0" style="font-size:12px">
                                <h5 class="m-0">{{popup.name}}</h5>
                                <p class="m-0">
                                    <span class="text-danger font-weight-bold" v-if="popup.entry_fee > 1">
                                        Rs:&#x20B9  {{popup.entry_fee}} /-
                                    </span>
                                    <span class="text-success font-weight-bold" v-else>
                                        Free Entry
                                    </span>
                                    {{popup.start_date | date}}</p>
...
export default {
    ...
    methods:{
        ...
        load_popup_ad(){
            axios.get('/api/event/list/popup_ad')
            .then(response => {
